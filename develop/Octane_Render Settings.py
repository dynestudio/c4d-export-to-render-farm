...
def Octane_Log_Data(): #thanks to the octane user mpazera for the base code
    ...
    if kernelLV == 0:
        ...
        #lista=["Geometric normals", "Shading normals"]
        #for i in range(24):
        #	if info_type == i:
        #		info_type = lista[i]
    ...
    if kernelLV == 3:
        ...
        max_samples = octane[c4d.SET_PMC_MAXSAMPLES] = int(data.GetReal(c4d.SET_PMC_MAXSAMPLES))
        max_samples = "Max. Samples = " + str(max_samples)
        diffuse_depth = octane[c4d.SET_PMC_DIFDEPTH] = int(data.GetReal(c4d.SET_PMC_DIFDEPTH))
        diffuse_depth = "Diffuse Depth = " + str(diffuse_depth)
        glos_depth = octane[c4d.SET_PMC_GLOSSYDEPTH] = int(data.GetReal(c4d.SET_PMC_GLOSSYDEPTH))
        glos_depth = "Glossy depth = " + str(glos_depth)
        ray_epsilon = octane[c4d.SET_PMC_RAYEPS] = float(data.GetReal(c4d.SET_PMC_RAYEPS))
        ray_epsilon = "Ray epsilon = " + str(ray_epsilon)
        Filter = octane[c4d.SET_PMC_FSIZE] = float(data.GetReal(c4d.SET_PMC_FSIZE))
        Filter = "Filter size = " + str(Filter)
        ...
        if ashadow == True:
            ...
            ashadow = "Alpha shadows: " + "Disabled"
        causticblur = octane[c4d.SET_PMC_CAUSTICBLUR] = float(data.GetReal(c4d.SET_PMC_CAUSTICBLUR))
        causticblur = "Cuastic blur = " + str(causticblur)
        GI_clamp = octane[c4d.SET_PMC_GICLAMP] = float(data.GetReal(c4d.SET_PMC_GICLAMP))
        GI_clamp = "GI clamp = " + str(GI_clamp)
        ...
        if keep_env == True:
            ...
            keep_env = "Keep environment: " + "Disabled"
        pter_power = octane[c4d.SET_PMC_PTERMPOWER] = float(data.GetReal(c4d.SET_PMC_PTERMPOWER))
        pter_power = "Path term. power = " + str(pter_power)
        explostrength = octane[c4d.SET_PMC_EXPLOSTRENGTH] = float(data.GetReal(c4d.SET_PMC_EXPLOSTRENGTH))
        explostrength = "Exploration strength = " + str(explostrength)
        dlimportance = octane[c4d.SET_PMC_DLIMPORTANCE] = float(data.GetReal(c4d.SET_PMC_DLIMPORTANCE))
        dlimportance = "Direct light importan = " + str(dlimportance)
        maxrejects = octane[c4d.SET_PMC_MAXREJECTS] = int(data.GetReal(c4d.SET_PMC_MAXREJECTS))
        maxrejects = "Max. rejects = " + str(maxrejects)
        parallelism = octane[c4d.SET_PMC_PARALLELISM] = int(data.GetReal(c4d.SET_PMC_PARALLELISM))
        parallelism = "Parallelism = " + str(parallelism)
        chunk_size = octane[c4d.SET_PMC_WORK_CHUNK_SIZE] = int(data.GetReal(c4d.SET_PMC_WORK_CHUNK_SIZE))
        chunk_size = "Work chunk size = " + str(chunk_size)
